...
function verifyTimestampedSignature(signature, message, timestamp) {
  ...
  if (!verifySignature(signature, message, address))
    throw 'Signature doesn\'t match address'
  if (!(message === `Cancel all loans for ${address} at ${timestamp}`))
    throw 'Message doesn\'t match params'
  if (!(currentTime <= (timestamp + 60)))
    throw 'Signature is stale'
  if (!(currentTime >= (timestamp - 120)))
    throw 'Timestamp is too far ahead in the future'
