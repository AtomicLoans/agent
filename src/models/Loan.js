...
const LoanSchema = new mongoose.Schema({
  ...
  status: {
    type: String,
    enum: ['QUOTE', 'POF_SET', 'REQUESTING', 'AWAITING_COLLATERAL', 'APPROVING', 'APPROVED', 'CANCELLING', 'CANCELLED', 'WITHDRAWN', 'REPAID', 'ACCEPTING', 'ACCEPTED', 'LIQUIDATING', 'LIQUIDATED', 'FAILED'],
    index: true
  },
  lastWarningSent: {
    type: Date
...
LoanSchema.index({ principal: 1, loanId: 1 }, { unique: true, partialFilterExpression: { principal: { $type: 'string' }, loanId: { $type: 'int' } } })
